...
EOF
  ...
  if is_alpine; then
    # 创建 OpenRC 服务
    # 创建启动包装脚本
    cat > /etc/gost/gost.sh <<'EOF'
#!/bin/sh
cd /etc/gost && exec /etc/gost/gost "$@"
EOF
    chmod +x /etc/gost/gost.sh

    # 创建 OpenRC 服务
    cat > /etc/init.d/gost <<'SERVICEEOF'
#!/sbin/openrc-run
name="gost"
description="Gost Proxy Service"
command="/etc/gost/gost.sh"
...
EOF
  ...
  echo "🔄 检查服务状态..."
  if is_alpine; then
    if rc-service gost status 2>/dev/null; then
      echo "✅ 安装完成，gost服务已启动并设置为开机启动。"
      echo "📁 配置目录: $INSTALL_DIR"
    else
      echo "❌ gost服务启动失败，请检查 /var/log/messages"
    fi
  else
